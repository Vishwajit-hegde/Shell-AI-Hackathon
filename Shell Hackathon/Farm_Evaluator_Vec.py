...
def getAEP(turb_rad, turb_coords, power_curve, wind_inst_freq,
            n_wind_instances, cos_dir, sin_dir, wind_sped_stacked, C_t,get_each_turbine_aep=0):
    ...
    # year_hours = 8760.0
    if get_each_turbine_aep:
        AEP = 8760.0*(power*wind_inst_freq.reshape(-1,1))/1e3
        return np.sum(AEP,axis=0)
    else:
        power   = np.sum(power, axis=1)
...
def checkConstraints(turb_coords, turb_diam,v=0):
    """
    -**-THIS FUNCTION SHOULD NOT BE MODIFIED-**-
    ...
    for i,turb1 in enumerate(turb_coords):
        for turb2 in np.delete(turb_coords, i, axis=0):
            prox = np.linalg.norm(turb1 - turb2) - 4*turb_diam
            if  prox<0:
                prox_constr_viol -= 1
                #break
    return prox_constr_viol
    # print messages
